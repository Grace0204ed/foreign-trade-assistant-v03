...
function detail(c){let wa=c.whatsapp_number||c.phone.replace(/\D/g,"");let quoteHistory=c.quote_history||[],latest=c.quote_summary?.latest||quoteHistory[0];return`<aside class="detail-panel"><button class="detail-close">×</button><div class="detail-title"><span class="avatar big grade-${c.grade.toLowerCase()}">${esc(c.name[0])}</span><div><h2>${esc(c.name)}<i class="grade grade-${c.grade.toLowerCase()}">${c.grade}级</i></h2><p>${esc(c.buyer_type)} · ${esc(c.country||"国家未填")}</p></div></div>
...
${latest?`<section class="latest-quote-card"><div><label>最新报价</label><strong>${esc(latest.quoteNumber)}${latest.version?` · V${latest.version}`:""}</strong><small>${fmt(latest.formalizedAt||latest.updatedAt||latest.date)} · ${esc(latest.type==="vehicle"?"新车组合报价":latest.documentType==="invoice"?"形式发票":"二手/常规报价")}</small></div><div><b>${esc(latest.currency)} ${Number(latest.total||0).toLocaleString()}</b><a href="/?${latest.type==="vehicle"?"vehicleQuote":"quote"}=${latest.id}">查看最新报价 →</a></div></section>`:""}
<section class="timeline quote-timeline"><div class="timeline-head"><h3>全部历史报价</h3><span>${quoteHistory.length} 份/版本</span></div>${quoteHistory.length?quoteHistory.map(q=>`<article class="quote-history-item ${q.isLatest?"latest":""}"><time>${fmt(q.formalizedAt||q.updatedAt||q.date)} ${q.isLatest?"· 最新报价":q.isLatestVersion?"· 当前版本":"· 历史版本"}</time><p><a href="/?${q.type==="vehicle"?"vehicleQuote":"quote"}=${q.id}">${esc(q.quoteNumber)}${q.version?` · V${q.version}`:""}</a><em>${esc(q.type==="vehicle"?"新车":q.documentType==="invoice"?"形式发票":"二手/常规")}</em></p><small>${esc(q.status)} · ${esc(q.currency)} ${Number(q.total||0).toLocaleString()} · ${Number(q.quantity||0)} 台/项</small>${q.machineSummary?`<small class="quote-machine">${esc(q.machineSummary)}</small>`:""}${q.sourceQuoteId?`<small>复制来源：${esc(q.sourceQuoteId)}</small>`:""}</article>`).join(""):'<div class="empty mini">该客户还没有报价记录</div>'}</section>
